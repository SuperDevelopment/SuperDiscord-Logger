...
class SlashCommands {
    ...
    async create(name, description, options, guildId) {
        ...
        if (cmd) {
            ...
            if (cmd.description !== description ||
                ...
                optionsChanged) {
                console.log(`SuperDiscord : Updating${guildId ? ' guild' : ''} slash command "${name}"`);
        ...
        if (commands) {
            console.log(`SuperDiscord : Creating${guildId ? ' guild' : ''} slash command "${name}"`);
    ...
    async delete(commandId, guildId) {
        ...
        if (commands) {
            ...
            if (cmd) {
                console.log(`SuperDiscord : Deleting${guildId ? ' guild' : ''} slash command "${cmd.name}"`);
                cmd.delete();
            }
        }
        return Promise.resolve(undefined);
