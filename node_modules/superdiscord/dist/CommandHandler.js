...
class CommandHandler {
    ...
    async setUp(instance, client, dir, disabledDefaultCommands, typeScript = false) {
        ...
        if (dir) {
            ...
            console.log(`SuperDiscord : Loaded ${amount} command${amount === 1 ? '' : 's'}.`);
    ...
    async registerCommand(instance, client, file, fileName, builtIn = false) {
        ...
        if (missing.length && instance.showWarns) {
            console.warn(`SuperDiscord : Command "${names[0]}" does not have the following properties: ${missing}.`);
        }
        if (testOnly && !instance.testServers.length) {
            console.warn(`SuperDiscord : Command "${names[0]}" has "testOnly" set to true, but no test servers are defined.`);
        }
        if (slash !== undefined && typeof slash !== 'boolean' && slash !== 'both') {
            throw new Error(`SuperDiscord : Command "${names[0]}" has a "slash" property that is not boolean "true" or string "both".`);
        }
        if (!slash && options.length) {
            throw new Error(`SuperDiscord : Command "${names[0]}" has an "options" property but is not a slash command.`);
        }
        if (slash && !(builtIn && !instance.isDBConnected())) {
            if (!description) {
                throw new Error(`SuperDiscord : A description is required for command "${names[0]}" because it is a slash command.`);
            }
            if (minArgs !== undefined && !expectedArgs) {
                throw new Error(`SuperDiscord : Command "${names[0]}" has "minArgs" property defined without "expectedArgs" property as a slash command.`);
            }
            if (options.length) {
                for (const key in options) {
                    ...
                    if (name !== lowerCase && instance.showWarns) {
                        console.log(`SuperDiscord : Command "${names[0]}" has an option of "${name}". All option names must be lower case for slash commands. SuperDiscord will modify this for you.`);
                    }
                    if (lowerCase.match(/\s/g)) {
                        lowerCase = lowerCase.replace(/\s/g, '_');
                        console.log(`SuperDiscord : Command "${names[0]}" has an option of "${name}" with a white space in it. It is a best practice for option names to only be one word. SuperDiscord will modify this for you.`);
                    }
