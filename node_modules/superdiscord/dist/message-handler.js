...
class MessageHandler {
    ...
    get(guild, messageId, args = {}) {
        ...
        if (!translations) {
            console.error(`SuperDiscord : Could not find the correct message to send for "${messageId}"`);
    ...
    getEmbed(guild, embedId, itemId, args = {}) {
        ...
        if (!items) {
            console.error(`SuperDiscord : Could not find the correct item to send for "${embedId}" -> "${itemId}"`);
            return 'Could not find the correct message to send. Please report this to the bot developer.';
        }
        const translations = items[itemId];
        if (!translations) {
            console.error(`SuperDiscord : Could not find the correct message to send for "${embedId}"`);
            return 'Could not find the correct message to send. Please report this to the bot developer.';
        }
        let result = translations[language];
        for (const key of Object.keys(args)) {
