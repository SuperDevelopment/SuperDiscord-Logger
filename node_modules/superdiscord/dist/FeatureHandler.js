...
class FeatureHandler {
    ...
    setup = async (dir, typeScript) => {
        ...
        if (amount > 0) {
            console.log(`SuperDiscord : Loading ${amount} listener${amount === 1 ? '' : 's'}...`);
            for (const [file, fileName] of files) {
                const debug = `SuperDiscord DEBUG > Feature "${fileName}" load time`;
            ...
            console.log(`SuperDiscord : Loaded ${amount} listener${amount === 1 ? '' : 's'}.`);
        }
    };
    registerFeature = (file, fileName) => {
        ...
        if (config) {
            ...
            if (missing.length && this._instance.showWarns) {
                console.warn(`SuperDiscord : Feature "${fileName}" has a config file that doesn't contain the following properties: ${missing}`);
            }
        }
        else if (this._instance.showWarns) {
            console.warn(`SuperDiscord : Feature "${fileName}" does not export a config object.`);
        }
        ...
        if (config && config.loadDBFirst === true) {
            console.warn(`SuperDiscord : config.loadDBFirst in features is no longer required. MongoDB is now connected to before any features or commands are loaded.`);
        }
