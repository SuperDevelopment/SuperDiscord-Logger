...
class FeatureHandler {
  ...
  private _instance: SuperDiscord
  ...
  private setup = async (dir: string, typeScript: boolean) => {
    ...
    if (amount > 0) {
      console.log(
        `SuperDiscord : Loading ${amount} listener${amount === 1 ? '' : 's'}...`
      )
      ...
      console.log(
        `SuperDiscord : Loaded ${amount} listener${amount === 1 ? '' : 's'}.`
      )
    }
  }
  ...
  private registerFeature = (file: any, fileName: string) => {
    ...
    if (config) {
      ...
      if (missing.length && this._instance.showWarns) {
        console.warn(
          `SuperDiscord : Feature "${fileName}" has a config file that doesn't contain the following properties: ${missing}`
        )
      }
    } else if (this._instance.showWarns) {
      console.warn(
        `SuperDiscord : Feature "${fileName}" does not export a config object.`
      )
    }
    ...
    if (config && config.loadDBFirst === true) {
      console.warn(
        `SuperDiscord : config.loadDBFirst in features is no longer required. MongoDB is now connected to before any features or commands are loaded.`
      )
    }
