...
const sendHelpMenu = (message: Message, instance: SuperDiscord) => {
...
module.exports = {
  ...
  init: (client: Client, instance: SuperDiscord) => {
  ...
  callback: (options: ICallbackObject) => {
    ...
    if (guild && !guild.me?.permissions.has('SEND_MESSAGES')) {
      console.warn(
        `SuperDiscord : Could not send message due to no permissions in channel for ${guild.name}`
      )
      return
    }
