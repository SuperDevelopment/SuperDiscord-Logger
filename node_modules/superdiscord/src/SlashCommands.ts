...
class SlashCommands {
  ...
  constructor(instance: SuperDiscord, listen: boolean, typeScript?: boolean) {
  ...
  public async delete(
    ...
    if (commands) {
      ...
      if (cmd) {
        console.log(
          `SuperDiscord : Deleting${guildId ? ' guild' : ''} slash command "${
            cmd.name
          }"`
        )
