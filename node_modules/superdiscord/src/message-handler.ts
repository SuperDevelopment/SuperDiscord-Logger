...
export default class MessageHandler {
  ...
  constructor(instance: SuperDiscord, messagePath: string) {
  ...
  get(
    ...
    if (!translations) {
      console.error(
        `SuperDiscord : Could not find the correct message to send for "${messageId}"`
      )
  ...
  getEmbed(
    ...
    if (!items) {
      console.error(
        `SuperDiscord : Could not find the correct item to send for "${embedId}" -> "${itemId}"`
      )
      return 'Could not find the correct message to send. Please report this to the bot developer.'
    }
    ...
    if (!translations) {
      console.error(
        `SuperDiscord : Could not find the correct message to send for "${embedId}"`
      )
      return 'Could not find the correct message to send. Please report this to the bot developer.'
    }
